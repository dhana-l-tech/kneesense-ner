...
- `src/lib/sensorSource.ts` — just the `SensorSource` interface the ESP32 link (`bleConnection.ts`) implements (see "Real ESP32 + BLE" below). There is deliberately no simulated/fake implementation: a screening tool must never silently substitute fabricated data for a missing sensor, so a capture page with no sensor connected shows a blocking error instead of starting (see `ErrorModal.tsx`).
...
- `src/lib/riskScoring.ts` — rule-based risk scoring per the project spec's weights (symptoms 30% / ROM 25% / movement quality 15% / mobility 20% / sensor-camera agreement 10%), producing a `low` / `moderate` / `high` category. The mobility component blends reported difficulty with the walk test's objective speed/pauses/assistance/gait-irregularity when available. Pure function, unit-verified against synthetic low/moderate/high cases and against good-walk/bad-walk cases. This is a **cross-check design, not a fusion** — the camera's angle never replaces or averages with the IMU's ROM measurement; it only produces the separate 10% agreement score (and is dropped entirely below 50% confidence, per spec). The IMU stays the primary signal for the 40% ROM + movement-quality weight.
- **No fallback data, ever** — there is no simulated/fake `SensorSource` in the app at all (removed entirely, not just unused). `ExerciseCapturePage.tsx`/`SitToStandPage.tsx` refuse to start a capture unless `isSensorConnected()` is true, showing a blocking `ErrorModal` ("Go to Sensor Pairing") instead. Every `exercise_captures` row still records a `data_source` column (`'ble'` / `'simulated'` / `'manual'`) — `'simulated'` can no longer actually be produced by the UI, but `computeAndSaveRiskScore()` (`riskScores.ts`) still throws `SimulatedCaptureError` if it ever sees one, as defense-in-depth against a future regression, rather than silently scoring fabricated data. `ResultsPage.tsx` would show a dedicated "reconnect the sensor and recapture" screen for that case.
- **Sensor error handling** — `bleConnection.ts` exposes a shared `onDisconnect()` subscription (one native BLE disconnect callback, fanned out to whichever screens care) so a mid-capture disconnect is caught immediately: the partial capture is discarded (not saved half-finished) and `ErrorModal.tsx` shows a clear "sensor disconnected — reconnect and redo" popup rather than the capture silently freezing. The same popup pattern covers a failed `startNotifications`/`write` call when starting a capture. `SensorPairingPage.tsx` shows the same modal for connect/calibrate failures instead of small inline text.
- **Low-power detection** — the board runs off USB from a power bank (confirmed with the project owner), which exposes no battery-voltage rail to sense directly, so real proactive "battery at X%" monitoring isn't possible without different hardware. What *is* implemented: the ESP32's own brownout detector force-resets the chip if the input rail sags too low, and the firmware reports on the next connect whether its last boot was one of those resets (new read-only status characteristic, `STATUS_CHAR_UUID` in `bleProtocol.ts`/the `.ino`) — `SensorPairingPage.tsx` shows an amber "possible low-power reset" popup (`ErrorModal`'s new `variant="warning"`) when that bit is set. This is retrospective (found out after reconnecting, not live during a capture) — see `firmware/README.md`'s "Low-power detection" section for the caveats and what a live/predictive version would need (a voltage-divider circuit on a spare ADC pin, not implemented).
...
- [`firmware/kneesense_esp32/kneesense_esp32.ino`](firmware/kneesense_esp32/kneesense_esp32.ino) — **single-sensor mode**: one MPU6050 (shin-mounted) → complementary filter → BLE notify, plus the calibration button and RGB LED/buzzer/vibration-motor feedback from the original hardware plan. The hardware moved from an earlier two-sensor (thigh+shin) design to one sensor; the app's `bleConnection.ts` treats the un-instrumented thigh as a fixed 0° reference rather than the analysis code needing a rewrite — see `firmware/README.md`'s "Single-sensor mode" section for exactly what that assumes and where it breaks (sit-to-stand). Wiring diagram and setup steps are in [`firmware/README.md`](firmware/README.md). **Not compiled or flashed in this session** — no Arduino toolchain is available here, so it needs to be verified on real hardware before trusting it; the BLE payload math (8-byte little-endian layout) is separately unit-verified on the app side.
...
- `src/lib/bleConnection.ts` — a persistent, module-level BLE connection (pair once after attaching the sensor, reuse across knee-extension + sit-to-stand, rather than reconnecting per exercise). Exposes `connect()`, `calibrate()`, `onDisconnect()` (shared disconnect-listener registry — see "Sensor error handling" above), and `createBleSensorSource()` (implements `SensorSource`, decodes the firmware's single-angle payload into the `{thighAngle: 0, shinAngle}` shape the rest of the app expects, and reports start/streaming failures via an `onError` callback instead of only `console.error`).
- `src/pages/SensorPairingPage.tsx` — the step between the questionnaire and the exercise captures: Connect → Calibrate, matching the spec's workflow order. The step itself is skippable (a health worker can navigate past it to look around the app), but skipping does **not** unlock captures — the exercise screens independently require a live connection and show `ErrorModal` if there isn't one. Calibration instructions specifically say to straighten the leg first (required for single-sensor mode's 0°-reference to be correct).

**What's verified vs. what needs real hardware:** the 8-byte angle payload's byte layout round-trips exactly (checked with a real `DataView`, matching what the ESP32's `memcpy` would produce). Live-tested the full pairing path: the connect button correctly reaches the real Web Bluetooth `requestDevice()` call (confirmed by the specific error it produces — a `SecurityError` about needing a genuine user gesture, which this automated test can't provide, not an API-missing or code error), and a failed connect attempt shows `ErrorModal` cleanly with no regression to the rest of the flow. The firmware itself and the live BLE data path can only be exercised with a real ESP32 + MPU6050, which this sandbox doesn't have — the C++ was written and reviewed carefully but not compiled; check the Serial Monitor output on first flash. Earlier bring-up reportedly used a generic BLE scanner app (nRF Connect / Serial Bluetooth Terminal) to confirm the ESP32 was broadcasting at all — that's a fine sanity check, but this app's own BLE stack (`bleConnection.ts`) is what actually needs to pair for the sensor data to reach the classification pipeline.
